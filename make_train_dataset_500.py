...
def process_file(gbff_file, chunk_file):
    ...
    identified_nucleotide_file = chunk_file.replace('genomic.chunk500.fna', 'identified_nucleotide.fa')
    identified_protein_file = chunk_file.replace('genomic.chunk500.fna', 'identified_protein.fa')
    ...
    for gbff_file_gz in gbff_list:
        ...
        for infile_gz in fna_list:
            if infile_gz.startswith(base_name):
                output_file = infile_gz.replace('genomic.fna.gz', 'genomic.chunk500.fna')
